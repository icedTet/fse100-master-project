...
export class ShapeManager extends EventEmitter {
  ...
  addShape(shape: Shape) {
    ...
    while (anyOverlapping) {
      ...
      for (let i = 0; i < this.shapes.size; i++) {
        ...
        if (this.isOverlapping(shapes[i], shape)) {
          anyOverlapping = true;
        } else all++;
      }
      ...
      if (anyOverlapping) {
        let x =
          shape.radius +
          Math.random() * (window.innerWidth / 2 - shape.radius) * 0.8;
        let y =
          shape.radius +
          Math.random() * (window.innerHeight - shape.radius) * 0.8;
    ...
    while (anyOverlapping) {
      ...
      for (let i = 0; i < this.shapes.size; i++) {
        // console.log(shape,shapes[i], this.isOverlapping(shape,shapes[i]))
        if (this.holeIsOverlapping(shapes[i], shape)) {
          anyOverlapping = true;
        } else all++;
      }
      if (all === this.shapes.size) anyOverlapping = false;
      if (anyOverlapping) {
        let x =
          (shape.radius + 50) * 1 +
          window.innerWidth / 2 +
          Math.random() * (window.innerWidth / 2 - shape.radius - 50) * 0.8;
        let y =
          (shape.radius + 50) * 1 +
          Math.random() * (window.innerHeight - shape.radius - 50) * 0.8;
  ...
  releaseShapeFromMouse() {
    if (this.draggedShape) soundReject.play();
    this.draggedShape = null;
  ...
  holeIsOverlapping(a: Shape, b: Shape) {
    ...
    let d = Math.sqrt(
      (a.destinationCenter.x - b.destinationCenter.x) ** 2 +
        (a.destinationCenter.y - b.destinationCenter.y) ** 2
    );
    return d <= a.radius + b.radius;
  }
}
